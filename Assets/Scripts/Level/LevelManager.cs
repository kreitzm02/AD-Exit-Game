...
public class LevelManager : MonoBehaviour
{
    ...
    [Header("START MODE")]
    [SerializeField][HideInInspector] private bool debugBypassLevelSystem = false;
    ...
    private void ApplyCurrentStep()
    {
        ...
        if (step.updateQuestText && !string.IsNullOrEmpty(step.newQuestText))
        {
            ...
            if (qc != null)
            {
                ...
                if (!suppressAutosave) qc.ShowQuestBar();
            }
        }

        AudioManager.Instance.PlayNewMusic(step.musicType);

        Debug.Log("[LevelManager] Step Activated: " + step.stepName);

        if (!suppressAutosave && currentStepIndex < 100) // no more saves after cellar escape in lvl3
            SaveCurrentGame();
    }
